                ...
                <div class="flex-grow-1 ps-2">
                    ...
                    <div class="d-flex align-items-center mb-2" style="width: fit-content;">
                        <form method="POST" action="{{ route('carrito.actualizar', $id) }}"
                            class="d-flex align-items-center me-1 form-actualizar-cantidad">
                            @csrf
                        ...
                        <div class="px-3 cantidad-numero" id="cantidad-{{ $id }}">{{ $item['cantidad'] }}
                        </div>

                        <form method="POST" action="{{ route('carrito.actualizar', $id) }}"
                            class="d-flex align-items-center ms-1 form-actualizar-cantidad">
                            @csrf
                    ...
                    <form action="{{ route('carrito.eliminar', $id) }}" method="POST"
                        class="form-eliminar-sidebar eliminar-item-form">
                        @csrf
                        <input type="hidden" name="desde_sidebar" value="1">
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    if (data.eliminado && data.producto_id) {
                        ...
                        const containerWelcome = document.getElementById('carrito-container-' + data
                            .producto_id);
                        if (containerWelcome) {
